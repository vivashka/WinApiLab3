...
bool forceNew = false; // если true, игнорируем сохранённое состояние
...
BOOL LoadState()
{
    ...
    if(!ReadFile(hFile, &file_cell_size, sizeof(file_cell_size), &readBytes, NULL) || readBytes != sizeof(file_cell_size))
    {
        CloseHandle(hFile);
        return FALSE;
    }
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow)
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        else if(lstrcmpW(argv[i], L"-reset") == 0)
        {
            forceNew = true;
        }
    }
    ...
    switch(uMsg)
    {
        ...
        case WM_PAINT:
        {
            ...
            for(int i = 0; i <= grid_width; i++)
            {
                ...
                LineTo(hdc, x, grid_height * cell_size);
            }
            ...
            for(int j = 0; j <= grid_height; j++)
            {
                ...
                LineTo(hdc, grid_width * cell_size, y);
            }
